...
The easiest way to issue certificates of insurance is via the SDK `apps` helper — see [TypeScript SDK → Application helpers](../sdk/typescript#application-helpers-apps).
...
This template is part of the identity system — see [Identity & Credentials](../identity) for the concept and the [IdentityAuth Certificate](../identity/identity-auth-certificate) reference for all metadata fields.
...
This template uses a custom transaction builder that calls the extension's `/build` endpoint instead of the standard certificate flow. See the [Tokenizable Certificate extension](../extensions/tokenizable-certificate) for the full lifecycle.
...
See the [Fractionized Certificate extension](../extensions/fractionized-certificate) for the full lifecycle.
